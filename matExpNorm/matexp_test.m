...
for i = 1:length(ts)
    ...
    y5(i) = norm(eA * v(:,1)); %this should be equal to the matrix norm
    %plot lower bd
    y2(i) = 1 - M*eps*t;
    y1(i) = norm(eA * v(:,1) - eA*vbig);

    % y3(i) = norm(expm(t.*A));
    y4(i) = norm(eA * vbig);
    y5(i) = norm(expm(t.*A)*(V(:,1)));
